...
def parse_args():
    parser = argparse.ArgumentParser(description="GDesigner AQuA unsupervised logprob stage")
    ...
    add_common_unsup_args(
        parser,
        dataset_name="aqua",
        unsup_data="ExpanData/unlabeled/AQuA.jsonl",
        stage1_checkpoint="result/checkpoints/aqua.pt",
        checkpoint_file="result/checkpoints/aqua_unsup.pt",
        metrics_file="result/aqua_unsup.jsonl",
    )
...
async def main():
    args = parse_args()
    eval_records = aqua_data_process(load_jsonl(args.dataset_json))
